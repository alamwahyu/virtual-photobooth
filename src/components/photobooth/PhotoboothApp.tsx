      ...
      <div className="mx-auto max-w-6xl space-y-7 sm:space-y-10">
        ...
        {state === "READY_TO_START" && layout && frame && (
          <section className="mx-auto max-w-xl space-y-5 rounded-lg border border-black/10 bg-white/75 p-5 text-center shadow-soft sm:p-6">
            ...
            <button type="button" onClick={openCameraIntro} className="touch-target inline-flex w-full items-center justify-center rounded-md bg-ink px-6 py-3 font-semibold text-white">
        ...
        {state === "REVIEW" && (
          <section className="space-y-5">
            ...
            <button type="button" onClick={compose} className="touch-target inline-flex w-full items-center justify-center rounded-md bg-ink px-6 py-3 font-semibold text-white sm:w-auto">Gunakan Foto Ini</button>
          </section>
        )}
    ...
    <div className="fixed inset-0 z-[60] grid place-items-center bg-black/50 px-4 py-6 backdrop-blur-sm">
      <section role="dialog" aria-modal="true" aria-labelledby="camera-intro-title" className="w-full max-w-md rounded-lg bg-white p-5 text-ink shadow-soft sm:p-6">
        ...
        <div className="mt-6 grid gap-3">
          <button type="button" onClick={onContinue} className="touch-target inline-flex items-center justify-center rounded-md bg-ink px-5 font-semibold text-white">
            <Camera className="mr-2 inline" size={18} />
            Lanjutkan ke Kamera
          </button>
          <button type="button" onClick={onChooseGallery} className="touch-target inline-flex items-center justify-center rounded-md border border-black/15 bg-white px-5 font-semibold text-ink">
            <ImagePlus className="mr-2 inline" size={18} />
            Pilih Foto dari Galeri
          </button>
          <button type="button" onClick={onCancel} className="touch-target inline-flex items-center justify-center rounded-md px-5 font-semibold text-black/60">
            Nanti Saja
          </button>
        </div>
      </section>
